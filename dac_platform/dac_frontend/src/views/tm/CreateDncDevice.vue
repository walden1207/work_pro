...
<script>
  ...
  import { fetchDncProtocolMap } from '@/api/tm'
  // import { fetchDncProtocolMap } from '@/mock/tm'
  import DncChoice from './components/DncChoice'
  import DncConfig from './components/DncConfig'
  ...
  export default {
    ...
    methods: {
      fetchProtocolList() {
        ...
        fetchDncProtocolMap(queryParam)
          .then(res => {
            this.protocolMap = res
          })
          .catch(msg => {})
      },
      handleChoiceClick(index) {
        if (this.protocolMap && this.templateList) {
